		...
		if enemies ~= nil then
			...
			for i, e in pairs(enemies) do
				...
				if self.ai.buildsitehst:isInMap(upos) then
					...
					enemy = UpdateEnemy(e)
					if enemy then self.ai.knownEnemies[e:ID()] = enemy
				end
			end
...
function LosHST:UpdateEnemy(enemy)
	-- game:SendToConsole("updating known enemies")
	local t = {} --a temporary table
	local id = enemy.unitID
	local ename = enemy.unitName
	local pos = enemy.position --if is died pos is nil -- exixtance check
	t.knownid = true
	t.mobile = self.ai.armyhst.unitTable[ename].speed > 0
	local moveType self.ai.armyhst.unitTable[e.unitName].mtype
	if not pos then
			...
			if GULS >= 7 then
				...
				elseif pos.y < 0 then
					...
					elseif t.moveType == 'sub' then
						...
						self:Warn('unespected moveType underWater',pos.x,pos.z,ename,moveType)
					end
				else
					t.surface = true
					if self.ai.maphst:IsUnderWater(pos) then
						t.water = true
					else
						t.ground = true
					end
				end
			elseif (GULS == 4 or GULS == 6)  and not self.ai.armyhst.unitTable[ename].stealth and not Spring.IsUnitInJammer ( id, self.ai.allyId )  then
				t.radar = true
				t.speed = Spring.GetUnitVelocity ( id ) --TODO keep dir and speed
				t.knownid = false
				t.knownEnemy = true
			elseif GULS == 2 and  t.mobile then
				t = nil --remove mobile just seing onetime
			elseif GULS == 0 then
				--totally unknow
				t = nil
			else
				self:Warn('unespected GULS response',GULS,id,pos)
			end
		end
	end
	return t
...
--[[
		...
		if los == 0 and self.ai.armyhst.unitTable[ename].isBuilding then
			-- don't remove from knownenemies if it's a building that was once seen --TODO chck nano too
